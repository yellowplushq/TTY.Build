...
struct MenuView: View {
    ...
    var body: some View {
        ...
        .frame(width: 360)
        .font(TTYBuildTheme.text)
        .background(TTYBuildTheme.canvas)
        .tint(TTYBuildTheme.content)
        .onAppear {
            permissions.refresh()
            #if DEBUG
            if !openedSettingsForTesting,
               ProcessInfo.processInfo.environment["TTYBUILD_OPEN_SETTINGS_FROM_MENU"] == "1" {
        ...
        VStack(alignment: .leading, spacing: 0) {
            if model.sessions.isEmpty {
                ...
                ForEach(model.sessions, id: \.id) { session in
                    SessionRow(
                        session: session,
                        onOpenInTerminal: { model.openSessionInTerminal(session.id) },
    ...
    var body: some View {
        HStack(spacing: 8) {
            Image(systemName: "terminal")
                .foregroundStyle(session.alive ? TTYBuildTheme.content : TTYBuildTheme.tertiaryContent)
                .frame(width: 16)
            VStack(alignment: .leading, spacing: 2) {
                Text(session.title)
                    .lineLimit(1)
            ...
            if session.alive {
                ...
                .help("Open in Terminal (takes over the session)")
            }
            Button {
                onClose()
            } label: {
