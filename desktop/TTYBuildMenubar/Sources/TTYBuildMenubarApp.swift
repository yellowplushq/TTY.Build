...
enum SettingsWindow: String, Codable, Hashable {
    ...
    static let id = "ttybuild-settings"
}

@main
struct TTYBuildMenubarApp: App {
    @NSApplicationDelegateAdaptor(TTYBuildAppDelegate.self) private var appDelegate
...
@MainActor
final class TTYBuildAppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if ProcessInfo.processInfo.environment["TTYBUILD_OPEN_SETTINGS"] == "1" {
            let opener = NSWindow(
            ...
            if let path = ProcessInfo.processInfo.environment["TTYBUILD_SETTINGS_SNAPSHOT"] {
    ...
    /// Managed hooks outlive the app bundle in ~/.tty.build/bin. Refresh the
    ...
    private func refreshManagedAgentHooks() {
        ...
        let reporterDestination = TTYBuildHome().hookReporterURL
    ...
    /// TTYBuild is a menu-bar service whose lifetime must not be tied to its
    /// Settings windows.
