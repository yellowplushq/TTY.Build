...
///
/// Installing an agent copies the bundled `ttybuild-hook` reporter to
/// `~/.tty.build/bin/ttybuild-hook` (a stable path that survives app relocation)
...
final class AgentHooksModel: ObservableObject {
    ...
    private let reporterDestination = TTYBuildHome().hookReporterURL

    /// The reporter binary embedded in the app bundle by the build.
    private var bundledReporter: URL? {
        guard let executable = Bundle.main.executableURL else { return nil }
        let url = executable.deletingLastPathComponent().appendingPathComponent("ttybuild-hook")
    ...
    func install(_ agent: HookInstaller.HookedAgent) {
        ...
        guard let bundled = bundledReporter else {
            lastError = "The ttybuild-hook helper is missing from this build."
            return
        }
        do {
