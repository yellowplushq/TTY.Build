...
struct SettingsView: View {
    ...
    private struct PermissionsPage: View {
        ...
        private static let explanation = """
            TTYBuild asks for these up front so programs running in remote \
            terminal sessions can use them without approval prompts appearing \
            on this Mac while you're away. TTYBuild itself never reads your \
            files, controls your Mac, or records your screen.
            """

        private static let footnote = """
            Grants apply to new sessions. macOS may ask to relaunch TTYBuild \
    ...
    private struct AgentsPage: View {
        ...
        private static let agentRows: [(
            ...
            (.copilot, "copilot-mark", "Copilot CLI",
             "Hook file in ~/.copilot/hooks/ttybuild.json."),
            (.grok, "grok-mark", "Grok",
             "Hook file in ~/.grok/hooks/ttybuild.json."),
            (.hermes, "hermes-mark", "Hermes",
             "Plugin in ~/.hermes/plugins/ttybuild-presence/."),
            ...
            (.omp, "omp-mark", "Oh My Pi",
             "Extension in ~/.omp/agent/extensions/ttybuild/."),
            (.opencode, "opencode-mark", "OpenCode",
             "Plugin in ~/.config/opencode/plugins/ttybuild-presence.js."),
            (.pi, "pi-mark", "Pi",
             "Extension in ~/.pi/agent/extensions/ttybuild/."),
        ]

        private static let explanation = """
            TTYBuild can watch coding agents running anywhere on this Mac — any \
        ...
        var body: some View {
            Form {
                Section {
                    ...
                    Text("TTYBuild stays available while its menu bar icon is visible.")
                        .foregroundStyle(TTYBuildTheme.secondaryContent)
                }
            }
            .formStyle(.grouped)
