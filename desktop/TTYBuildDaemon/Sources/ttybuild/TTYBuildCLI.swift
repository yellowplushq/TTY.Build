...
private func fail(_ message: String) -> Never {
    FileHandle.standardError.write(Data("ttybuild: \(message)\n".utf8))
    Foundation.exit(1)
}
...
struct Serve: ParsableCommand {
    ...
    func run() throws {
        ...
        print("pair from iOS with: ttybuild pair")
...
struct Kill: ParsableCommand {
    ...
    @Argument(help: "Session id (see `ttybuild ls`).")
    var id: Int
...
struct HooksInstall: ParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "install",
        abstract: "Copy the reporter to ~/.tty.build/bin and write agent hook settings."
    )
    ...
    func run() throws {
        ...
        guard FileManager.default.fileExists(atPath: source.path) else {
            fail("reporter binary not found at \(source.path) — build ttybuild-hook first")
        }
        do {
...
struct HooksUninstall: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        abstract: "Remove ttybuild-managed hook entries from agent settings."
    )
...
struct HooksStatus: ParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "status",
        abstract: "Report whether ttybuild-managed hooks are installed."
    )
    ...
    func run() throws {
        ...
        case .outdated: print("outdated — run `ttybuild hooks install \(slug)`")
        }
    }
}
