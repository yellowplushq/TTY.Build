...
public enum RemoteManagement {
    ...
    /// host cannot name its own version (e.g. headless `ttybuild serve`).
    ...
    public enum RemoteError: Error, CustomStringConvertible {
        ...
        /// No `ttybuild-hook` binary next to the running executable.
        ...
        public var description: String {
            ...
            case .unknownAgent(let slug):
                "unknown agent \"\(slug)\""
            case .reporterMissing:
                "the ttybuild-hook helper is missing from this build"
            }
        }
    }
