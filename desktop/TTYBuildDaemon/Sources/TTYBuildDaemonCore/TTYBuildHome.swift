...
/// `~/.tty.build` — v2 service config, host identity, and control-socket paths.
/// The directory can be overridden with the `TTYBUILD_HOME` environment variable
/// (used by tests and by CI to avoid touching the real home directory).
public struct TTYBuildHome: Sendable {
    public let directory: URL

    public init(directory: URL? = nil) {
        if let directory {
            self.directory = directory
        } else if let override = ProcessInfo.processInfo.environment["TTYBUILD_HOME"],
                  !override.isEmpty {
    ...
    public var socketPath: String { directory.appendingPathComponent("ttybuild.sock").path }
    public var sessionCounterURL: URL { directory.appendingPathComponent("session-counter") }
    /// Binaries shipped into the home directory (the ttybuild-hook reporter),
    ...
    /// The public `ttybuild` CLI (docs/EXCLUSIVE_ATTACH_DESIGN.md §7),
    /// installed here by the menu bar app so "Open in Terminal" scripts and
    /// PATH symlinks survive app relocation.
    public var cliURL: URL { binDirectory.appendingPathComponent("ttybuild") }
    ...
    /// check-to-mutation race in `ttybuild pair`.
