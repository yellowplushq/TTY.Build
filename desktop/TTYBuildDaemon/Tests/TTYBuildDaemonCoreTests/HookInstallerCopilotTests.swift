...
/// Copilot's whole-file installer at ~/.copilot/hooks/ttybuild.json: canonical
...
final class HookInstallerCopilotTests: XCTestCase {
    ...
    private var fileURL: URL {
        home.appendingPathComponent(".copilot/hooks/ttybuild.json")
    }

    override func setUpWithError() throws {
        home = FileManager.default.temporaryDirectory
            .appendingPathComponent("ttybuild-copilot-\(UUID().uuidString)", isDirectory: true)
    ...
    func testOutdatedWhenReporterPathChanges() throws {
        try HookInstaller.install(for: .copilot, reporterPath: "/old/ttybuild-hook", home: home)
