...
final class HookInstallerKimiTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        home = FileManager.default.temporaryDirectory
            .appendingPathComponent("ttybuild-kimi-\(UUID().uuidString)", isDirectory: true)
    ...
    func testOutdatedWhenReporterPathChanges() throws {
        try install(reporter: "/old/ttybuild-hook")
        XCTAssertEqual(try state(), .outdated)
        try install()
        XCTAssertEqual(try state(), .installed)
        let config = try readConfig()
        XCTAssertFalse(config.contains("/old/ttybuild-hook"), "old blocks replaced")
    }
    ...
    func testTomlEscapingRoundTrips() throws {
        ...
        let awkward = #"/Users/me/"weird dir"/ttybuild\hook"#
        ...
        XCTAssertTrue(config.contains(#"ttybuild\\hook"#), "backslash escaped on disk")
    }
