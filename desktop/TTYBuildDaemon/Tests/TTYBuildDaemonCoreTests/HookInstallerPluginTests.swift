...
final class HookInstallerPluginTests: XCTestCase {
    ...
    private let reporter = "/Users/me/.ttybuild/bin/ttybuild-hook"

    override func setUpWithError() throws {
        home = FileManager.default.temporaryDirectory
            .appendingPathComponent("ttybuild-plugins-\(UUID().uuidString)", isDirectory: true)
    ...
    private func runSingleFileLifecycle(
        ...
        try HookInstaller.install(for: agent, reporterPath: "/old/ttybuild-hook", home: home)
        XCTAssertEqual(try state(agent), .outdated, file: file, line: line)
        try HookInstaller.install(for: agent, reporterPath: reporter, home: home)
        XCTAssertEqual(try state(agent), .installed, file: file, line: line)
    ...
    func testOpenCodeLifecycle() throws {
        try runSingleFileLifecycle(.opencode, marker: "// ttybuild-managed-hook")
    }

    func testOpenCodeContent() throws {
        try HookInstaller.install(for: .opencode, reporterPath: reporter, home: home)
        XCTAssertTrue(path(for: .opencode).hasSuffix(".config/opencode/plugins/ttybuild-presence.js"))
        let text = try content(.opencode)
        XCTAssertTrue(text.contains("export const TTYBuildPresence = async"))
    ...
    func testOpenCodeGeneratedJavaScriptParses() throws {
        let url = home.appendingPathComponent("ttybuild-presence.mjs")
    ...
    func testRefreshManagedGeneratedPluginsUpdatesOnlyOwnedFiles() throws {
        try HookInstaller.install(
            for: .opencode, reporterPath: "/old/ttybuild-hook", home: home
        )
        try HookInstaller.install(
    ...
    func testOmpLifecycle() throws {
        try runSingleFileLifecycle(.omp, marker: "/* ttybuild-managed-extension */")
    }

    func testPiLifecycle() throws {
        try runSingleFileLifecycle(.pi, marker: "/* ttybuild-managed-extension */")
    }

    func testOmpContent() throws {
        try HookInstaller.install(for: .omp, reporterPath: reporter, home: home)
        XCTAssertTrue(path(for: .omp).hasSuffix(".omp/agent/extensions/ttybuild/index.ts"))
    ...
    func testPiContent() throws {
        ...
        XCTAssertTrue(path(for: .pi).hasSuffix(".pi/agent/extensions/ttybuild/index.ts"))
    ...
    private var hermesDir: URL {
        home.appendingPathComponent(".hermes/plugins/ttybuild-presence", isDirectory: true)
    }
    ...
    func testHermesFreshInstall() throws {
        ...
        XCTAssertTrue(module.hasPrefix("# ttybuild-managed-hook\n"))
    ...
    func testHermesStateRequiresBothFilesByteEqual() throws {
        ...
        try HookInstaller.install(for: .hermes, reporterPath: reporter, home: home)
        XCTAssertEqual(
            try HookInstaller.state(for: .hermes, reporterPath: "/old/ttybuild-hook", home: home),
            .outdated
        )
    }
